  ...
  if (!leanCanvasData) {
    return (
      <div className="text-center">
        <p className="text-lg text-gray-600">リーンキャンバスの生成に失敗しました。</p>
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        {selectedProductName && (
          ...
          >
            プロダクト名: <span className="text-primary">{selectedProductName.name}</span>
          </motion.div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {canvasBlocks.map((block, index) => (
          ...
          >
            <Card className={`h-full border-2 ${block.borderColor} bg-gradient-to-br ${block.color} hover:shadow-lg transition-all duration-300`}>
                ...
                <ul className="space-y-2">
                  {block.content.map((item, itemIndex) => (
                    <motion.li
                      ...
                      className="text-sm text-gray-700 bg-white/50 p-2 rounded-md border border-white/30"
                    >
                      • {item}
                    </motion.li>
                  ))}
