...
export abstract class TaskProcessor<TRequest, TResponse> {
  ...
  async process(body: any): Promise<TResponse> {
    ...
    if (!normalizer.validate(normalizedData)) {
      throw new Error(`Difyから${this.getTaskName()}データが返されませんでした`);
    }
...
export class PersonaTaskProcessor extends TaskProcessor<DifyPersonaRequest, DifyPersonaResponse> {
  validateRequest(body: any): body is DifyPersonaRequest {
    return body && typeof body.keyword === 'string' && body.keyword.trim() !== '';
  }
  ...
  async process(body: any): Promise<DifyPersonaResponse> {
    ...
    return {
      personas: normalizedData
    } as DifyPersonaResponse;
  }
}

export class BusinessIdeaTaskProcessor extends TaskProcessor<DifyBusinessIdeaRequest, DifyBusinessIdeaResponse> {
  ...
  async process(body: any): Promise<DifyBusinessIdeaResponse> {
    const normalizedData = await super.process(body);
    return {
      business_ideas: normalizedData
    } as DifyBusinessIdeaResponse;
  }
}

export class ProductNameTaskProcessor extends TaskProcessor<DifyProductNameRequest, DifyProductNameResponse> {
  ...
  async process(body: any): Promise<DifyProductNameResponse> {
    const normalizedData = await super.process(body);
    return {
      product_names: normalizedData
    } as DifyProductNameResponse;
  }
}

export class CanvasTaskProcessor extends TaskProcessor<DifyCanvasRequest, LeanCanvasData> {
...
export class TaskProcessorFactory {
  static create(task: string, difyClient: DifyApiClient): TaskProcessor<any, any> {
    switch (task) {
      case 'persona': 
